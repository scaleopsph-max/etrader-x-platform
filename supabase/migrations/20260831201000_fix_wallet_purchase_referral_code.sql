...
declare
  ...
  submitted_referral_code text := nullif(upper(trim($2)), '');
  new_order_id uuid;
  new_payment_id uuid;
...
begin
  ...
  values (buyer_id, selected_plan.id, 'approved', selected_plan.price_amount, selected_plan.currency, submitted_referral_code, 'Paid using wallet balance')
  ...
  if submitted_referral_code is not null then
    ...
    where p.referral_code = submitted_referral_code
      and p.id <> buyer_id;
